...
export function TranscriptView() {
  ...
  // для них актуальна проблема «токен проскакивает мимо fillerWords». Фразы из
  // replacements теперь корректно обрабатываются движком (см. rules.ts §2a —
  // токены внутри заменённой фразы получают decoration will-replace и не идут в
  // OOV), но filler_phrases движок только удаляет, не «заменяет», поэтому их
  // токены всё ещё нужно скрывать здесь. Мемоизация: пересчёт только при
  // изменении словарей.
